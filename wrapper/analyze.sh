...
# heatmap
eval $wrapper/heatmap_barplot.sh $RibosePreferenceAnalysis $scripts/heatmap_barplot $scripts/refseq/hg38_chrM.fa $bed_folder $order $output/heatmap_barplot 2>&1 > $output/logs/heatmap_barplot.log &

# Strand split figures
eval $wrapper/draw_strand_split.sh $RibosePreferenceAnalysis $scripts/heatmap_barplot $scripts/refseq $output/bed_split $order $output 2>&1 > $output/logs/strand_split.log &

# enriched zones
eval $scripts/enriched_zone/enriched_zone_analysis.py $bed_folder/*.bed $scripts/refseq/hg38_chrM.fa.fai $order -o $output/enriched_zone/chrM 2>&1 > $output/logs/enriched_zone.log &                                                        
